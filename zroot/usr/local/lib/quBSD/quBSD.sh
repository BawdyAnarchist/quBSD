...
stop_jail() {
	...
	# Requires input, $1 to designate target jail
	...
	if jls -j "$_jail" > /dev/null 2>&1 ; then  
		...
		if ! jail -qr "$_jail" ; then
			jail -qR "$_jail"
			sh ${QBDIR}/exec.poststop "$_jail" > /dev/null
		fi
	fi
}
...
restart_jail() {
	...
	if stop_jail "$_jail" > /dev/null 2>&1 ; then 
		jail -qc  "$_jail" > /dev/null 2>&1
	fi
}

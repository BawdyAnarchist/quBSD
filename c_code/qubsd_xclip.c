 ...
 * different X displays, primarily between the host and Xephyr sessions.
 * It operates via a single, short-lived background process.
 *
 * It is not a generic tool and is tailored for the quBSD environment.
 *
 * Compilation:
 *   cc qubsd_xclip.c -o qubsd_xclip -lX11
 *  OR
...
int main(int argc, char *argv[]) {
    if (argc != 2) {
        fprintf(stderr, "Usage: %s copy | paste\n", argv[0]);
        return 1;
    }
...
fprintf(stderr, "DEBUG: Daemon started (PID: %d). Lock and FIFO setup complete. Proceeding to display detection.\n", getpid());

    BrokerState state = {0};
    g_state = &state; // Set global pointer for signal handler
    strncpy(state.lock_file_path, lock_file_path, sizeof(state.lock_file_path) - 1);
    snprintf(state.fifo_path, sizeof(state.fifo_path), FIFO_TEMPLATE, getuid());
    ...
    if (!temp_dpy) {
        fprintf(stderr, "Error: Could not open host display '%s'\n", host_display);
        free(host_display);
        cleanup_and_exit(1);
    }
    ...
    if (!state.source_display) {
        fprintf(stderr, "Error: Failed to determine source display.\n");
        cleanup_and_exit(1);
    }

    state.source_dpy = XOpenDisplay(state.source_display);
    if (!state.source_dpy) {
        fprintf(stderr, "Error: Could not open source display '%s'\n", state.source_display);
        cleanup_and_exit(1);
    }

    fetch_source_targets(&state);
    if (state.num_targets == 0) {
        fprintf(stderr, "Error: Could not fetch any clipboard targets from source '%s'.\n", state.source_display);
        cleanup_and_exit(1);
    }
fprintf(stderr, "DEBUG: Source display detected: '%s'. Connection successful: %s. Targets fetched: %d.\n",
        state.source_display ? state.source_display : "NULL",
        state.source_dpy ? "YES" : "NO", state.num_targets);
...
void daemonize(void) {
    ...
    if (!log_fp) {
        // Fallback if log can't be opened, point to /dev/null
...
char* detect_host_display(void) {
    ...
    fprintf(stderr, "Warning: Could not detect host display, falling back to ':0'\n");
    return strdup(":0");
}
...
char* get_active_window_display(Display *dpy) {
    ...
    if (XGetWindowProperty(dpy, DefaultRootWindow(dpy), net_active_window_atom, 0, 1, False, XA_WINDOW,
        ...
        fprintf(stderr, "Fatal: Could not determine active window.\n");
        return NULL;
    }
...
void fetch_source_targets(BrokerState *state) {
    ...
    if (pid == 0) { // Child process
        ...
        fprintf(stderr, "DEBUG CHILD (fetch_targets): Child process attempting execvp. CMD: xclip -display %s -o -selection clipboard -t TARGETS\n", state->source_display);
        ...
        fprintf(stderr, "DEBUG PARENT (fetch_targets): xclip child for TARGETS exited with status %d.\n", WEXITSTATUS(status));
        close(pipe_fds[0]);
    }
}

void arm_for_paste(BrokerState *state) {
    fprintf(stderr, "DEBUG: arm_for_paste called (from FIFO). Attempting to take ownership on new destin_display.\n");
    ...
    if (strcmp(state->source_display, state->destin_display) == 0) {
        fprintf(stderr, "INFO: arm_for_paste: Source '%s' and destination '%s' are same, doing nothing.\n", state->source_display, state->destin_display);
        return;
    }
    ...
    state->broker_window = XCreateSimpleWindow(state->dest_dpy, DefaultRootWindow(state->dest_dpy), 0, 0, 1, 1, 0, 0, 0);
    ...
    fprintf(stderr, "DEBUG: Ownership on destin_display '%s' acquired: %s.\n",
...
void main_event_loop(BrokerState *state) {
    ...
    while (1) {
        ...
        int x11_fd = -1;
fprintf(stderr, "DEBUG: Loop iteration. Time left: %ld sec. destin_display is %s. X_FD: %d\n",
        TIME_TO_LIVE - (time(NULL) - state->last_activity_time),
        state->destin_display ? "set" : "not set",
        state->dest_dpy ? ConnectionNumber(state->dest_dpy) : -1);
        if (state->dest_dpy) {
        ...
        struct timeval tv;
        tv.tv_sec = TIME_TO_LIVE - (current_time - state->last_activity_time);
        tv.tv_usec = 0;
        ...
        if (FD_ISSET(state->fifo_fd, &fds)) {
            char buffer[16];
            if (read(state->fifo_fd, buffer, sizeof(buffer)) > 0) {
...
void handle_selection_request(BrokerState *state, XSelectionRequestEvent *req) {
    ...
    fprintf(stderr, "DEBUG: SelectionRequest received for target '%s' (Atom ID: %lu).\n",
    ...
    if (req->target == atom_targets) {
        fprintf(stderr, "DEBUG: handle_selection_request: Responding to TARGETS request with %d targets.\n", state->num_targets);
        XChangeProperty(state->dest_dpy, req->requestor, req->property, XA_ATOM, 32, PropModeReplace, (unsigned char *)state->targets, state->num_targets);
    ...
    if (target_name) {
        ...
        if (tmp_fd != -1) {
            ...
            if (pid == -1) {
                ...
                fprintf(stderr, "DEBUG CHILD (selection_request): Child process attempting execvp. CMD: xclip -display %s -o -selection clipboard -t %s\n", state->source_display, target_name);

                char *argv[] = {"xclip", "-display", state->source_display, "-o", "-selection", "clipboard", "-t", target_name, NULL};
                execvp("xclip", argv);
                        ...
                        if (size > 0) {
                            ...
                            if (buffer) {
                                if (fread(buffer, 1, size, fp) == (size_t)size) {
                                    XChangeProperty(state->dest_dpy, req->requestor, req->property, req->target, 8, PropModeReplace, buffer, size);
                                    notify_event.property = req->property;
                                }
                                free(buffer);
                            }
                ...
                fprintf(stderr, "DEBUG: xclip child for '%s' exited with status %d. Data transfer to property %s.\n",
                        target_name, WEXITSTATUS(status),
                        (WIFEXITED(status) && WEXITSTATUS(status) == 0 && notify_event.property != None) ? "succeeded" : "FAILED");
            }
            unlink(tmp_file_path);
        }
    }

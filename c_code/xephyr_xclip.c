 ...
 *
 * Add this line to xinitrc:  while sleep 2 ; do xephyr_xclip; ; done & 
 *
...
/* ---------------- Debug macro ------------------------------------ */
#ifdef DEBUG
  static const char *dlog_path = "/tmp/xephyr_xclip.log";
...
static void process_x_events(Nest *n, XEvent *event)
{
    switch (event->type) {
        ...
        case SelectionRequest:                           // Window wants to paste from clipboard owner
            ...
            if (req->target == n->atom_targets) {        // Always allow TARGETS queries harmless & expected
        ...
        default:                     // X11 extension for owner change. Not core-protocol, needs xfixes_base
